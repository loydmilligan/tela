...
- **Purpose:** Show the plan ladder without hype, and make **Atlas a flagship reason to be on a paid tier** — not a footnote. The product meters per-account tiers (personal + org); pricing makes the ladder legible. The thesis still holds — tiers change *limits* (spaces, storage, the monthly AI-answer allowance, and how much Atlas generation you get), never the product: the wiki, Atlas, search, ask-your-docs, and the agent connector are on every plan.
  - **PRICING DECISION (2026-06-27, set with the user):** prices are UNCHANGED — Atlas raises the *value*, not the price (v0 adoption-first). The new per-tier dimension is an **Atlas source cap** (how many git repos / Jira projects Atlas keeps live + refreshed), a real column in the backend `plans` table (migration `0052_plan_atlas_sources`) so the landing mirrors a source of truth. Caps: Free 1 · Plus 5 · Org Free 1 · Team 20 · Enterprise unlimited. (Mirrored in the backend `plans` table and **enforced** at source-create time — adding a source past the cap returns `402 quota_exceeded`, counted across all the account's projects.)
- **Headline (H2):** `Simple plans. Your markdown either way.`
- **Lede:** `A personal account and every organization carry their own tier. Every plan includes the whole product — Atlas, semantic search, ask-your-docs, and the agent connector. Tiers only change the limits: spaces, storage, how many AI answers a month, and how much Atlas generation you get.`
- **Atlas-as-flagship band (lead the section with it):** `Atlas is on every plan — point it at a git repo or Jira project and it writes a cited, coverage-checked wiki and keeps it fresh. Paid tiers raise how much you can generate.` (The headline upgrade reason; render it prominently, above or beside the cards.)
- **Trial note (demoted — quiet fine-print under the tiers, NOT a pill in the head):** `No card to start — every new account gets a 30-day Plus trial, then settles onto your plan.` Purchase leads; the trial reassures, it doesn't headline. (Real: registration auto-applies a 30-day `personal_plus` trial that downgrades to free at expiry.) **Decision (2026-06-27, with the user):** the trial was too upfront on plan-click — the old early-access dialog is gone and the head pill is demoted to this footnote.
- **Plan CTAs (self-serve billing is live via Polar):** the two **Free** tiers → `Get started` → `/register` (free signup). **Paid** tiers → `Get Plus` / `Get Team` → the in-app upgrade page (`/settings?tab=billing`, login-gated; checkout is one click there). **Enterprise** → `Get in touch` (mailto). No dialog — a plan click goes straight to purchase/signup.
- **Dollar pricing (final — mirror the backend `plans` table):** Free `$0`; Personal Plus `$8/mo`; Org Team `$6/seat/mo`; Enterprise `Custom`.
- **Yearly billing (added 2026-06-28; reworked same day, with the user):** a **Monthly/Yearly toggle** sits **directly above the plan cards** (just under the Atlas flagship band, hidden until JS), so it reads as part of the ladder rather than a stray control. **Yearly is the DEFAULT** (no-JS sees yearly too — no mismatch). The card shows the **per-month equivalent** of the annual price as the headline number, with "Billed annually — $X/yr, save $Y" on a sub-line. **Discounts are per-tier, chosen so the per-month is a clean number** (2026-06-28 decision, with the user — avoids an awkward `$6.67`): Personal Plus `$6/mo` (= `$72/yr`, save `$24`, **25% off**), Org Team `$5/mo per member` (= `$60/member/yr`, save `$12`, 2 months free). Because the discount is no longer uniform, the toggle pill reads **"Save up to 25%"** (the per-card sub-lines carry the exact figures). Monthly view shows the true monthly price (`$8`, `$6/member`) with no sub-line. Free + Enterprise have no yearly option (toggle leaves them unchanged). The paid CTA carries `&interval=year` by default so the in-app checkout opens on yearly; switching to Monthly strips it. Backend: `plans.price_cents_yearly` + the `<plan>@year` Polar products (see docs/billing.md). Full yearly/monthly amounts mirror the backend `plans` table; the per-month figure is annual ÷ 12, display-only.
- **Personal tiers (cards):** `Free` (`$0` · 3 spaces · 100 pages/space · 100 MB · 50 AI answers/mo · **1 Atlas source**) · `Plus` (`$8/mo` · 25 · 1,000 · 5 GB · 1,000 AI answers/mo · **5 Atlas sources**).
- **Organization tiers (cards):** `Free` (`$0` · 10 · 500 · 1 GB · 5 members · 50 AI answers/mo · **1 Atlas source**) · `Team` *(recommended — the one earned-indigo card)* (`$6/seat/mo` · 100 · unlimited pages · 50 GB · 50 members · 2,000 AI answers/mo · **20 Atlas sources**) · `Enterprise` (`Custom` · unlimited everything, incl. Atlas sources; `Get in touch`).
- **Every-plan-includes (checklist band — tiers change limits, never features):** `**Atlas — generate & refresh a cited, coverage-checked wiki from a git repo or Jira project** · Semantic search + ask your docs, with citations · MCP connector for Claude & ChatGPT · Local folder sync over WebDAV · Real-time multiplayer editing · SSO, organizations & per-space roles · Plain markdown you own — export anytime.`
...
- **Honesty line:** every number (spaces, storage, AI answers/month, Atlas sources, dollar prices) mirrors the backend `plans` table — the source of truth. The AI allowance meters generative LLM calls (ask-your-docs); semantic *search* itself isn't capped. Atlas generation also spends LLM + embedding budget; on the hosted instance it draws on the same managed-AI path, and self-hosting you bring your own LLM + embedder. The Atlas source cap is a published per-tier limit, enforced at source-create time (a source past the cap returns 402). Self-serve billing is live (Polar, merchant of record): a paid-plan CTA takes you to in-app checkout; new accounts still start free with the 30-day Plus trial.
