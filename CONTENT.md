...
- **Dollar pricing (final — mirror the backend `plans` table):** Free `$0`; Personal Plus `$8/mo`; Org Team `$6/seat/mo`; Enterprise `Custom`. Display-only (no billing engine yet).
