...
- **Lede:** `tela makes real spreadsheets — live formulas, conditional formatting and number formats, with CSV and XLSX in and out. And because tela is agent-native, your agent can build the whole sheet for you: ask Claude or ChatGPT to "turn this CSV into a Q3 budget with totals and over-budget flags" and it does.`
...
- **Two surfaces:** the **on-page** Pricing section carries the **cloud ladder only**; the dedicated **`/pricing`** page adds the full self-host editions detail (Community / Enterprise / Commercial license), the cloud-vs-self-host matrix, and the core-vs-Enterprise feature line (component `SelfHostPricing.astro`).
...
- **Dollar pricing (mirror `docs/editions-and-pricing.md` + the backend `plans` table):** Free `$0`; Personal `$8/mo`; Team `$10/seat/mo`; Enterprise `Custom`.
...
- **Cloud ladder (cards — one unified per-seat ladder):** `Free` (`$0` · 1 Atlas source · 50 AI answers/mo · unlimited pages & spaces · the whole wiki/sheets/decks/sync; `Get started`) · `Personal` (`$8/mo` · 5 Atlas sources · 1,000 AI answers/mo · custom domain & public spaces) · `Team` *(recommended — the one earned-indigo card)* (`$10/seat/mo` · 20 Atlas sources · 2,000 AI answers/mo pooled · roles & member management) · `Enterprise` (`Custom` · unlimited Atlas & AI · SSO/SCIM/audit · advanced roles & governance · priority support + SLA; `Get in touch`).
- **Every-plan-includes (checklist band — tiers change limits, never the core product):** `**Atlas — generate & refresh a cited, coverage-checked wiki from a git repo or Jira project** · Semantic search + ask your docs, with citations · MCP connector for Claude & ChatGPT · Local folder sync over WebDAV · Real-time multiplayer editing · Organizations & per-space roles · Plain markdown you own — export anytime.` (SSO/SCIM/audit are **Enterprise**, not every-plan.)
...
- **Self-host editions (`/pricing` only — `SelfHostPricing.astro`):** `Community` (free, AGPL-3.0 · the whole product · BYO AI · community support; `Get the code` → GitHub) · `Enterprise` *(earned-indigo)* (`from $12/user/mo`, billed annually · everything in Community + SSO/SAML/OIDC, SCIM, audit, advanced RBAC, premium Atlas connectors, priority support + SLA · license key; `Talk to us`) · `Commercial license` (flat annual · the full open core under a commercial license, AGPL relief, no EE/key; for legal/compliance constraints; `Contact us`).
- **Honesty line:** every cloud number (Atlas sources, AI answers/month, dollar prices) mirrors `docs/editions-and-pricing.md` + the backend `plans` table. The AI allowance meters generative LLM calls (ask-your-docs); semantic *search* itself isn't capped. **Self-host is BYO AI on every edition** — point tela at your own OpenAI-compatible LLM + embedder. Enterprise features live in a proprietary `ee/` module unlocked by an offline-verifiable license key; the open core stays AGPL. **Status: this editions model is design-locked but NOT yet built in the backend/Polar — do not deploy the landing ahead of that work (checkout would mismatch).**
