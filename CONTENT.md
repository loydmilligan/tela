...
- **Rows:**
  ...
  - **Confluence** — **wins:** `Atlas turns a git repo or a Jira project into a cited, coverage-checked wiki, agents search and write it by meaning, and your content is portable markdown you can run yourself — no enterprise install, no per-seat pricing.` · **better:** `Deep two-way Jira workflow integration (tela documents from Jira; it doesn't drive Jira), granular permissions, and governance at thousand-user scale. Atlassian shops have reasons to stay.`
  - **Obsidian** — **wins:** `Built for a team: live multiplayer, a shared server with SSO and roles, and an agent API — not a single-player vault you sync by hand.` · **better:** `Local-first single-user is its whole point; the plugin library and graph view are unmatched. Solo? Hard to beat.`
...
- **Yearly billing (added 2026-06-28, with the user):** a **Monthly/Yearly toggle** sits under the lede (hidden until JS — no-JS shows monthly). Yearly = **2 months free** (10× monthly): Personal Plus `$80/yr` (save `$16`), Org Team `$60/seat/yr` (save `$12/seat`). Free + Enterprise have no yearly option (toggle leaves them unchanged). The toggle swaps the displayed amount/period + shows a per-card "Save $X/yr" line + a "2 months free" pill; the paid CTA carries `&interval=year` so the in-app checkout opens on yearly. Backend: `plans.price_cents_yearly` + the `<plan>@year` Polar products (see docs/billing.md). Numbers mirror the backend `plans` table.
