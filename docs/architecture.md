...
### Auth & access
- Session cookie `tela_session` (HttpOnly, SameSite=Lax, `Secure` when `TELA_PUBLIC_BASE_URL` is https). argon2id passwords. Bootstrap admin via `TELA_ADMIN_USERNAME`/`_PASSWORD`/`_EMAIL` (idempotent; unset password → generated + logged; `_EMAIL` is pre-confirmed and also backfills an existing admin).
