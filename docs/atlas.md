...
> Status: **Phase 1 done; Phase 2 backend done** — the full generation machinery is ported, wired, and green: `EngineStore` (Postgres, tested), LLM client (chat→`TELA_LLM_URL`, embed→`rag.Embedder` via the `EmbedFunc` seam), in-process publisher (`atlasPublisher`, direct-SQL upsert + prune + reindex, tested), the run executor + SSE hub + `ResumeDangling` (boots clean). **Remaining in Phase 2:** the HTTP API surface (source CRUD, trigger, run+SSE, coverage — behind `atlasSpaceManageErr`/`requireMembership`) and the live 1-1 diff against a deployed atlas reference. This is the developer/architecture contract for bringing the standalone `atlas` system natively into tela. It is the source of truth for the build; update it as phases land. End-user docs go to the tela Docs space (space 16) per `CLAUDE.md`.
