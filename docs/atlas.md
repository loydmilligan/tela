...
> Status: **Phases 1–4 done; UI first cut done — all on `feat/atlas`.** Backend: ported engine (`EngineStore` Postgres, chat→`TELA_LLM_URL`, embed→`rag.Embedder` via `EmbedFunc`), in-process publisher, executor + SSE + `ResumeDangling`, HTTP API (gating tested), **delta runs + freshness scheduler + drift** (Phase 4). UI: the generation console (`/spaces/$spaceId/atlas` — sources/run/live-SSE/coverage), `StatusBadge` + `CoverageGauge` owned primitives, and a "Generation" entry point in the space header. `cmd/atlasdiff` is the 1-1 fidelity harness. **Remaining:** Phase 5 (Jira source + secret store), Phase 6 (notifications + MCP tools + docs), a console visual-QA pass, and the **complete live 1-1 run at the very end** (compass → Atlas space 51 reference; models `qwen3:30b-a3b-instruct-2507-q4_K_M` + `qwen3-embedding:0.6b`; baseline 64 files / 77 surface / 433 chunks / 13 pages / 100% must-cover). This is the developer/architecture contract for bringing the standalone `atlas` system natively into tela. It is the source of truth for the build; update it as phases land. End-user docs go to the tela Docs space (space 16) per `CLAUDE.md`.
