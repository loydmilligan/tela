...
### Phase 3 — prompts + completions + elicitation ❌ DROPPED (research-driven)
Host-polish research (2026-06) found Claude.ai / ChatGPT **don't surface** prompts
or completions to users, **elicitation is "not widely supported"**, and
**sampling is deprecated (2026-07-28)**. All low/negative ROI for a consumer
connector → cut. The hosts reward: tools + output schemas + accurate
annotations/titles + pagination + **interactive widgets**. See §13.
...
### Phase 5 — OAuth (Connect button) — **AS = WorkOS AuthKit (decided)**

Architecture: tela = OAuth **Resource Server**; WorkOS AuthKit = **Authorization
Server** in **Standalone Connect ("OAuth Bridge")** mode — WorkOS owns the OAuth
protocol surface but bounces the user to **tela's own login**, tela authenticates
against its Postgres, then calls WorkOS `…/authkit/oauth2/complete` with
`user.id = tela users.id`. Result: the issued JWT's **`sub` IS the tela user id**
...
Open unknowns to verify against a real instance: exact claim keys (`email`/scope/org), JWKS signing alg (RS256 vs ES256), that Standalone puts the tela id in the **access token** `sub` (not just id_token), whether redirect URIs self-register via CIMD/DCR.
...
- ⬜ Pagination — IN. Cursor + `next_cursor` on `list_pages` / `search` / `search_bodies` / `semantic_search` (tool-result pagination, ours to implement; `list_spaces` stays unpaginated — bounded by membership)
...
## 13. Host-polish playbook (research-driven, 2026-06)

What Claude.ai / ChatGPT actually reward (verified against primary docs):
**tools + structuredContent/output-schemas + accurate annotations & titles +
pagination + interactive widgets (MCP Apps).** Prompts, completions, elicitation,
...
- ⬜ **ChatGPT `search` + `fetch` aliases** — exact OpenAI shape unlocks Deep
  ...
  getPageCore. **[VERIFY exact field set vs live Apps SDK docs before shipping.]**
- ⬜ **First widget — Page Reader card** (`get_page`): the single biggest
  "feels native" win. `ui://tela/page-view` resource + `_meta.ui.resourceUri`
  (standard) + `openai/outputTemplate` (ChatGPT alias) + `openai/widgetCSP`.
...
- ⬜ **Search-results widget** (clickable hits → `callTool('get_page')`).

**Tier 3 (publish/differentiate):**
- ⬜ Write-confirm diff widget (`update_page`/`delete_page`).
- ⬜ Space/page tree browser widget (`setWidgetState`, ChatGPT-only — degrade on Claude).
- ⬜ Directory submissions: Claude (clau.de/mcp-directory-submission — logo, docs,
  OAuth2.1+PKCE, MCP Inspector validation, test creds) + ChatGPT app submission.

**Widget mechanics (when building Tier 1.3+):** MCP Apps shipped 2026-01-26 (the
Apps-SDK ⊕ MCP-UI convergence); clients = Claude web+desktop, ChatGPT, Goose, VS
Code. Mechanism: tool `_meta.ui.resourceUri` → `ui://` HTML resource → host renders
sandboxed iframe with JSON-RPC-over-postMessage bridge; widget reads
`structuredContent`. **Mid-migration / [VERIFY] before coding:** the widget MIME
(`text/html;profile=mcp-app` vs legacy `text/html+skybridge`), the exact
