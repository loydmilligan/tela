...
func telaDeckPreamble(m *deckManifestDoc) string {
	...
	b.WriteString("- **Fence every slide's frontmatter** with `---` above and below it: a slide is `---` / `layout: …` (+ its fields) / `---`, and the next slide follows the same way. Most tahta slides are pure frontmatter with an empty body (cover, section, statement, stats, define, steps, panels, vs, metric, end…), so between two slides you'll have the first slide's closing `---`, a blank line, then the next slide's opening `---`. That is correct — do NOT collapse it to a single shared `---`. The mistake that silently breaks a deck: writing bare `layout:`/`title:` keys right after a `---` with no opening fence — Slidev then renders those keys as body text and the whole deck mis-parses. After authoring, call `lint_deck`; an agent write with structural errors is rejected on save.\n")
	if len(m.Modules) > 0 {
