...
func (s *Server) GetSpaceAccess(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.DB.QueryContext(r.Context(), `
		SELECT u.id, u.username, u.email, sm.role AS role,
		       'direct' AS source_kind, NULL::text AS source_name
		...
		SELECT u.id, u.username, u.email, sg.role,
		 ...
		 WHERE sg.space_id = $2 AND sg.principal_kind = 'org'
		UNION ALL
		SELECT u.id, u.username, u.email, sg.role,
		 ...
		 WHERE sg.space_id = $3 AND sg.principal_kind = 'group'`, spaceID, spaceID, spaceID)
	if err != nil {
