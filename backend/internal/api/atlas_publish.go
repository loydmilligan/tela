...
// withSummary clones the shared props and stamps a page-specific standfirst plus
// summary_lock, so the auto-summarizer skips atlas pages entirely (it already
// honors summary_lock) instead of re-deriving — often mis-deriving — a summary
// the drafting agent already wrote. A blank summary (model omitted the marker
// and nothing was planned) leaves props untouched, so that page falls through to
// the auto-summarizer as normal.
func withSummary(base map[string]any, summary string) map[string]any {
...
func (p *atlasPublisher) Publish(ctx context.Context, rc *engine.RunContext) error {
	...
	rootSummary := fmt.Sprintf("Coverage overview for %s, generated by atlas; each source is documented on its own page.", name)
	rootID, changed, err := p.upsert(ctx, tx, rc.Source.ID, "__root__", p.parentPageID, rootTitle, renderOverview(rc, name), withSummary(props, rootSummary), nil)
	if err != nil {
	...
	for i := range rc.Art.Pages {
		...
		ord := pg.Order
		id, changed, err := p.upsert(ctx, tx, rc.Source.ID, pg.Slug, &rootID, pg.Title, pg.Body, withSummary(props, pg.Summary), &ord)
		if err != nil {
